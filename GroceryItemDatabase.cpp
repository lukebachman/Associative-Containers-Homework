...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  try
  {
	...
	double price = 0.0;
		while(!fin.eof())
		{ 
			fin >> std::quoted(upc) >> ignore(',') >> std::quoted(brand) >> ignore(',')
				>> std::quoted(name) >> ignore(',') >> price;
			GroceryItem groceryitem(name, brand, upc, price);
			data.insert({ upc, groceryitem });
...
GroceryItem* GroceryItemDatabase::find(const std::string& upc)
{
	...
	auto search = data.find(upc); //STL map::find() function 
	if (search != data.end()) 
	{
		return &(search->second); //returns an address to a groceryitem
	} 
	else 
	{
		return nullptr;
